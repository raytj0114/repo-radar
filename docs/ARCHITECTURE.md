...
- 一面と違い、本文を**Suspenseでストリーミングしない**（意図的）: この面は同一ページの
  Server Actionで毎回再レンダーされるが、Next 16.2 は「Suspense境界を含むページ」への
  action応答ストリームを正常終了させず、クライアントが更新を受け取れない
  （`next start` で決定的に再現。詳細は `favorites/page.tsx` のコメント）。
  既定表示の本文はDB1クエリのみ＝GitHub呼び出しゼロなので、ブロッキングでも体感は変わらない
