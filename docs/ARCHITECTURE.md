...
- `promptVersion` はプロンプト世代（1 = 構造化以前 / 2 = 構造化JSON / 3 = 新聞常体・読点区切りの見出し・最終行は解釈）。
  TTL無しのキャッシュのままプロンプトを進化させるための世代管理で、既存行の再生成は管理者操作としてのみ行う
  （旧世代の行が縮刷版に残るのは新聞のアーカイブとして正しい）
- **文体契約（promptVersion 3以降）: `lines` は常体（だ・である）の完全文（句点終わり）・最終行は事実でなく解釈**。
  マークダウン記法は使わない（紙面はマークダウンを解釈せずそのまま印字する）。
  以後の消費箇所はこれを前提にする（短信 `composeBriefs` は末尾句点を落として採字、読み下し `toSpokenSummary` と
  lead-article の句点補完はどちらも冪等で、旧世代の句点なし行と混在しても壊れない）
- プロンプト調整時の標準検証は `scripts/sample-release-summary.mts`（実装と同一経路で実リリースを
  数本生成し文体契約を機械チェック。**DBには書かない**。プレビューOAuth不通でも回せる）
...
- 一面と違い、本文を**Suspenseでストリーミングしない**（意図的）: Suspense境界を含むページへの
...
**同一ページのServer Actionを持つ画面にSuspense境界（ページ内・ルートを問わない）を置くと、
action応答がクライアントへ届かずUIが永久にpendingで固まる**（Next 16.2.10）。
サーバー側は書き込み・再レンダーまで完了しているのに、`useTransition` のpendingが解けない。

Issue #47 で**同一ビルド**に対し 画面の形状 × 輸送層 を各20試行（計240試行）実測した。
輸送層は `next start` の前段に自己署名TLSのリバースプロキシを置き、ALPNで切り替えた。

| 画面の形状                                     | Suspense境界 | 素のHTTP/1.1 | TLS+HTTP/1.1 | TLS+HTTP/2 |
| ---------------------------------------------- | ------------ | ------------ | ------------ | ---------- |
| `/favorites`（再レンダー＝DB1クエリ）          | **なし**     | **20/20**    | **20/20**    | **20/20**  |
| `/favorites?star=1`（＝GitHub最大4リクエスト） | **なし**     | **20/20**    | **20/20**    | **20/20**  |
| `/favorites` の本文をSuspenseで包んだ版        | ページ内     | **4/20**     | **5/20**     | **19/20**  |
| `/repos/[owner]/[name]`                        | ルート       | **12/20**    | **7/20**     | **20/20**  |
| `/trending`                                    | ルート       | **11/20**    | **12/20**    | **20/20**  |

- **境界の有無が主因子**。境界の無い形状は120/120成功、ある形状はHTTP/1.1で4〜12/20
- **「再レンダーが重いから」ではない**。境界なしのままGitHub取得を4本足しても60/60通る
- **prefetch本数とも相関しない**。最も落ちる `/repos/[owner]/[name]` の `<Link>` prefetchは
  12本しかなく、最多の購読面（46〜65本）は境界が無ければ通る
  ＝接続数上限の食い潰し（上流 vercel/next.js #96109 の説明）だけでは説明できない
- **輸送層は強い修飾要因**。h2なら19〜20/20まで戻る。TLS経由のHTTP/1.1が素のHTTP/1.1と
  同成績なので、改善はプロキシ挿入ではなく多重化そのものによる。
  ただし**h2でも消えない**（ページ内境界で1/20）ので「HTTP/1.1限定」では説明できない
- サーバー側は**240/240でコミット済み**。壊れるのはUIだけ
- actionのPOSTは**成功セルでも同じ時刻にabortされる**（送信〜30ms・応答ヘッダ〜50ms・abort〜70ms、
  UI更新は〜130ms）。サーバーが閉じない応答streamの後始末なので、`net::ERR_ABORTED` は症状ではない
...
（Issue #49 の実測: `--repeat-each=10` で境界ありは 13/20 が赤、撤去後は `npm run e2e` が
5回連続で緑）。**リロード後のDB確認では検出できない**（サーバー側は常に書き込み済み）ので、
押した直後の表示変化を見ること
