...
**設計制約（#41 で全画面を紙面化する際）**: 同一ページでServer Actionを使う画面には
Suspense境界を置かない。`loading.tsx` によるルート境界も同じ扱い。ストリーミングしてよいのは
**同一ページのactionを持たない画面**（一面 `/` はこれに当たる。ログアウトは遷移なので該当しない）。
本番がHTTP/2でも例外にしない。

**既知の未修正**: `/trending` と `/repos/[owner]/[name]` は `(main)/(chrome)/loading.tsx` の
ルート境界を持つため、この制約に**既に違反している**。本番（Vercel = HTTP/2）では20/20なので
露見していないが、`next start`・セルフホストでは踏む。E2Eに購読トグルの往復テストが無く
検出できていないので、直すときは**先に往復のE2Eを足して落ちることを確認する**こと
