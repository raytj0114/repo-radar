...
| ----------- | -------------- | ----------- | ----- | --- | ------ | ---------- |
| `/`         | ダッシュボード | --          | --    | --  | --     | --         |
| `/trending` | トレンド       | 82          | 2.1 s | 0   | 640 ms | 2026-07-26 |

2026-07-26計測の注意点（Chrome DevTools Lighthouse 13.3.0 / Mobile）:

- 拡張機能が有効な通常プロファイルで計測したため、Lighthouseが「Chrome拡張がページ読み込みに悪影響」と警告。`/trending` のTBT 640msのうち約300msは拡張機能（DeepL）由来の長タスクであり、実力値はこれより良い可能性が高い。改善効果の判定に使う際は拡張機能を無効化したプロファイルで再計測すること
- 認証済み `/login` も計測: Performance 90 / LCP 1.7 s / CLS 0 / TBT 20 ms（Speed Index 13.0 sの異常値は拡張由来の描画遅延で、スコアを押し下げている）
- ダッシュボード（`/`）は未計測
