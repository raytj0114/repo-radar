...
export function buildReleaseSummaryPrompt(source: ReleaseSummarySource): string {
  ...
  return [
    ...
    `- lines: 記事本文をちょうど${SUMMARY_LINE_COUNT}行。各行は箇条書きの断片ではなく、句点「。」で終わる完全な一文として書いてください（各行60字程度まで。「・」などの記号を行頭に付けない）`,
    `  - 1〜${SUMMARY_LINE_COUNT - 1}行目: 破壊的変更・新機能・重要な修正を優先した事実の要点`,
    '  - 最終行: 事実の繰り返しではなく解釈。この変更が開発者に何を意味するかを、リリースノートから読み取れる範囲で一文にしてください（型の例: 「この慎重さに16系の設計思想が見える。」）',
